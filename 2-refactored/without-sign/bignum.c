...
/* function to print the list: every digit, most significant first */
int print_list(Dlist *head)
{
	...
	while (head != NULL) // traverse head -> tail
	{
		printf("%d", head->data);
		head = head->next;
	}
...
/* function to read one operand string into a digit list.
 * This is the unsigned engine, so a leading '+' or '-' is rejected as
 * invalid rather than interpreted. */
static int parse_one(const char *str, Dlist **head, Dlist **tail)
{
	int i = 0; // local index, private to this operand

	if (str[0] == '\0') // an empty operand is invalid
	{
		printf("Invalid number\n");
		return FAILURE;
	}

	while (str[i] != '\0') // walk every character
	{
		if (str[i] < '0' || str[i] > '9')
		{
...
/* function to load both operands into their lists */
int digit_to_list(Dlist **head1, Dlist **tail1, Dlist **head2, Dlist **tail2, char *argv[])
{
	...
	// parse each operand independently
	if (parse_one(argv[1], head1, tail1) == FAILURE)
	{
		return FAILURE;
	}
	if (parse_one(argv[3], head2, tail2) == FAILURE)
	{
		return FAILURE;
	}
 ...
 * I compare magnitudes first so sub_magnitude always gets the larger number,
 * and I report a negative true result through *negative. */
				...
				Dlist **headR, Dlist **tailR, int *negative, char *argv[])
{
	...
	*negative = 0;
	...
	if (cmp == 0)
	{
		// equal operands cancel exactly to zero
		insert_at_first(headR, tailR, 0);
		return SUCCESS;
	}
	if (cmp > 0)
	{
		// num1 > num2, ordinary positive result
		sub_magnitude(*tail1, *tail2, headR, tailR);
	}
	else
	{
		// num1 < num2, the result is negative so I subtract the other way round
		*negative = 1;
...
/* function to multiply the two operands using schoolbook long multiplication */
...
/* function to divide the two operands using long division.
