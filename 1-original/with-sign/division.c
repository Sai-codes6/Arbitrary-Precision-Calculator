...
int division(Dlist **head1, Dlist **tail1, Dlist **head2, Dlist **tail2, Dlist **headR, Dlist **tailR, char *sign1, char *sign2, char *signR, char *argv[])
{
	...
	char divsign;  // declare local variable
	...
	else if (*sign1 == '+' || *sign2 == '+')   // if either one of the sign is +ve
	{
		*signR = '+';   // result sign will be +ve
	}
	else if (*sign1 == '-' || *sign2 == '-')  // if either one of the sign is -ve
	...
	divsign = *signR;  // store the result sign in divsign
	...
	while (flag)   // loop until flag is set
	{
		...
		{
			subtraction(head1, tail1, head2, tail2, headR, tailR, sign1, sign2, signR, argv);   // function call to subtract two lists
